...
class Solution:
    def outerTrees(self, trees: List[List[int]]) -> List[List[int]]:
        ...
        def anti_clock(a, b, c):
            return product([b[0] - a[0], b[1] - a[1]],
                           [c[0] - a[0], c[1] - a[1]]) > 0

        for i in range(len(trees)):
            while len(stack) >= 2 and anti_clock(trees[stack[-2]],
                                                 trees[stack[-1]], trees[i]):
        ...
        for i in range(len(trees) - 1, -1, -1):
            ...
            while len(stack) > upper and anti_clock(
                    trees[stack[-2]], trees[stack[-1]], trees[i]):
                stack.pop()
            stack.append(i)
